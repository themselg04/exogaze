...
func _ready():
	if etiqueta:
		...
		etiqueta.position = Vector3(0, etiqueta_altura, 0)  # Coloca la etiqueta sobre el planeta
...
func escalar_planeta(escala: float) -> void:
	...
	if etiqueta:
		...
		etiqueta.position = Vector3(0,etiqueta_altura / escala,0)
